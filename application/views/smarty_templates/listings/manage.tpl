            ...
            {if $items == TRUE}
                {foreach from=$items item=item}
                    ...
                    <div class="card bg-light card-body col-row">
                        <div class="row">
                            <div class='col-row'>{url type="anchor" url=$smarty.capture.t_item_url text=$smarty.capture.t_item_img attr=""}</div>
                            <div class='col-row'>{url type="anchor" url=$smarty.capture.t_item_url text=$item.name|escape:"html":"UTF-8" attr=""}<br/>{$item.price_f} <br /> {$item.description_s|escape:"html":"UTF-8"}</div>
                            <div class='col-row'>
